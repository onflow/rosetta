...
func (s *Store) ExportAccounts(filename string) {
	...
	err := s.db.View(func(txn *badger.Txn) error {
		...
		for {
			...
			data[acct] = append(data[acct], binary.BigEndian.Uint64(key[9:]))
			it.Next()
		}
		return nil
	})
...
func (s *Store) HashForHeight(height uint64) ([]byte, error) {
	...
	if err != nil {
		if err == badger.ErrKeyNotFound {
			return nil, fmt.Errorf("indexdb: no data found for block height %d", height)
		}
		return nil, err
	}
	return hash, nil
...
func (s *Store) Index(height uint64, hash []byte, block *model.IndexedBlock) error {
	s.mu.RLock()
	latest := s.latest
	s.mu.RUnlock()
