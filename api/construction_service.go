...
func (s *Server) ConstructionMetadata(ctx context.Context, r *types.ConstructionMetadataRequest) (*types.ConstructionMetadataResponse, *types.Error) {
	...
	opts, xerr := s.getConstructOpts(r.Options)
	if xerr != nil {
		return nil, xerr
	}
	if opts.Inner {
		...
		if opts.SequenceNumber == -1 {
			acct, err := client.Account(ctx, opts.Payer)
			if err != nil {
				return nil, wrapErr(errInternal, err)
			}
			if len(acct.Keys) != 1 {
...
func (s *Server) ConstructionPayloads(ctx context.Context, r *types.ConstructionPayloadsRequest) (*types.ConstructionPayloadsResponse, *types.Error) {
	opts, xerr := s.getConstructOpts(r.Metadata)
	if xerr != nil {
		return nil, xerr
	}
	payer := opts.Payer
...
func (s *Server) ConstructionSubmit(ctx context.Context, r *types.ConstructionSubmitRequest) (*types.TransactionIdentifierResponse, *types.Error) {
	...
	if err != nil {
		// TODO(tav): Distinguish between non-fatal and fatal errors.
...
func (s *Server) getConstructOpts(md map[string]interface{}) (*model.ConstructOpts, *types.Error) {
	val, ok := md["protobuf"]
	if !ok {
		return nil, wrapErrorf(
			errInvalidConstructOptions,
...
func (s *Server) parseIntent(ops []*types.Operation) (*txnIntent, *types.Error) {
	...
	intent := &txnIntent{}
	switch typ {
	case opCreateAccount, opCreateProxyAccount, opDeploy:
		for _, op := range ops {
		...
		if typ == opCreateProxyAccount || typ == opDeploy {
			if len(ops) > 1 {
				return nil, wrapErrorf(
					errInvalidOpsIntent,
